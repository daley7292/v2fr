<template>
  <div class="container">
    <a-card class="register-card" :style="guestConfig?.is_email_verify==1?'height: 520px;':''">
      ...
      <a-form layout="vertical" ref="registerForm" :model="register" @submit="submitRegister">
        <a-form-item field="email" >
          <a-input v-model="register.email" :placeholder="t('register.form.email')"/>
        </a-form-item>
        <a-form-item v-if="guestConfig?.is_email_verify==1">
        ...
        <a-form-item v-if="guestConfig?.is_email_verify==1">
          <a-input v-model="register.email_code" :placeholder="t('register.form.email_code')"/>
        </a-form-item>
        <a-form-item>
        ...
        <a-form-item>
          <a-input v-model="register.redeem_code" :placeholder="t('register.form.redeem_code')"/>
        </a-form-item>
...
import { AuthRegister, AuthUnificationRegister, GuestCommConfig, SendEmailVerify } from "@/api/login";
...
const submitRegister = async ({
  ...
  AuthUnificationRegister(values).then(r=>{
    if (r.data){
