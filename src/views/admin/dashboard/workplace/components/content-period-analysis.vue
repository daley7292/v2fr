<template>
  <a-spin :loading="loading" style="width: 100%">
    <Chart style="width: 100%; height: 320px" :option="chartOption" />
  </a-spin>
</template>
  ...
  const fetchData = async () => {
    ...
    try {
      await queryOrders().then((res) => {
        ...
        //
        //   // 根据类型分配数据到对应数组
        //   if (item.type === i18n.t('dashboard.commission.nubmer')) {
        //     type1Values.value.push(item.value);
        //   } else if (item.type === i18n.t('dashboard.commission.amount')) {
        //     type2Values.value.push(item.value);
        //   } else if (item.type === i18n.t('dashboard.payment.number')) {
        //     type3Values.value.push(item.value);
        //   } else if (item.type === i18n.t('dashboard.payment.amount')) {
        //     type4ValuesDayNew.value.push(item.value);
        //   }
        // });
      });
      const count = ref(0)
      const colors = [
        "#246EFF", "#00B2FF", "#e86550", "#ff0017", "#774a3e",
        "#c96d45", "#8db76b", "#2e3832", "#3c4a66", "#956d2d",
        "#89e2b0", "#6b47a8"
      ];
